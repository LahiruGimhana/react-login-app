...
const Console = (props) => {
    ...
    let session=sessionStorage.getItem('userLogginSessionData');

    const logout = () => {
        // setdetails({ name: "", email: "" });
        sessionStorage.removeItem('userLogginSessionData');
        console.log("Logout");
        history.push('/login');
      }
    ...
    const getData=()=> {
        let data=sessionStorage.getItem('userLogginSessionData');
    ...
    return (
        <div className="container">
            <h3 >Console login</h3>
            <h4 >Welcome, <span>{session.name}</span></h4>
